...
// State holds iteration of coin state
type State struct {
	remaining int
	coins     []int
	change    []int
...
// Change0 return array of coin used
func Change0(coins []int, target int) (result []int, err error) {
	queue := []State{{target, coins, []int{}}}
	var state State
	for 0 < len(queue) {
